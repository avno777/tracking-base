...
import productModel from '../models/database/inventories.model'
...
const CustomerService: IProductService = {
  ...
  async getData(req: Request) {
    ...
    const _data = await productModel.find({ $and: conditions }).skip(skip).limit(limit).lean().exec()
    const count = await productModel.countDocuments({ $and: conditions })
  ...
  async getDataById(req: Request) {
    try {
      ...
      const data = await productModel.findById(dataId)
      return data
    } catch (error) {
      logger.error('Error creating data:', error) // Xử lý lỗi cụ thể
      throw error
  ...
  async updateDataById(req: Request) {
    try {
      ...
      const data = req.body
      const updatedData = await productModel.findByIdAndUpdate(dataId, data, { new: true })
      return updatedData
    } catch (error) {
      logger.error('Error creating data:', error) // Xử lý lỗi cụ thể
      throw error
  ...
  async deleteDataById(req: Request) {
    try {
      const dataId = req.body.id ? req.body.id : req.params.id
      await productModel.findByIdAndDelete(dataId)
    } catch (error) {
      logger.error('Error creating data:', error) // Xử lý lỗi cụ thể
      throw error
    }
